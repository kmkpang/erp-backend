...
const { Employee, Position, Salary_pay, Department, Leaving, Overtime } = require('./model/employeeModel');
const { Product, productType, productCategory, Transaction, Expense } = require('./model/productModel');
const {
    ...
    try {
        ...
        const insert = async (modelName, model, records) => {
            ...
            try {
                ...
                const pk = Object.keys(attributes).find(key => attributes[key].primaryKey && attributes[key].autoIncrement);

                if (pk) {
                    const seqValues = records.map(r => r[pk]).filter(id => Number.isInteger(id));
                    ...
                    if (maxId > 0) {
                        ...
                        try {
                            ...
                            await sequelize.query(`SELECT setval(pg_get_serial_sequence('${tableName}', '${pk}'), ${maxId + 1}, false);`);
                        } catch (seqErr) {
                            console.log(`Could not reset sequence for ${tableName}. It might not use a standard sequence name. Error: ${seqErr.message}`);
                        }
                    }
                }
        ...
        await insert('QuotationSaleDetails', Quotation_sale_detail, data.QuotationSaleDetails);
